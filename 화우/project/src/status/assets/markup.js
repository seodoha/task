...
jQuery(document).ready(function() {
	...
	function progressDashboard() {
		...
		$.each($lists, function(i) {
			...
			$item.find('.current').text( $list.find('.complete').length + $list.find('.modify').length );
			$item.find('.total').text( $list.find('.state').length );
			$item.find('.percent').text( Math.floor(($list.find('.complete').length + $list.find('.modify').length) / $list.find('.state').length * 100) + '%' );
