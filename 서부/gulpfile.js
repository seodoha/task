...
const paths = {
	...
	assets:{
		...
		mov:{
			src: './project/src/assets/mov/**/*',
			dest: './project/dist/assets/mov/'
		},
		sprite:{
...
// 브라우저 싱크 실행(화면 새로고침)
function liveServer(){
	browserSync.init( null, { proxy: 'http://localhost:5050' , port: 5051 });
};
...
function watchFiles() {
	...
	watch('./project/src/assets/js/lib/*.js', series([libaryJS]));
	...
	watch(paths.assets.mov.src, series([libaryMovs]));
...
exports.build = series(parallel(clean, htmlInclude, jsCompile, mainJsCompile, libaryJS, scssCompile, libaryImages, spritesImage, libaryMovs, libaryWsg, libaryFonts)); // 전체 빌드 task
exports.semi = series(parallel(htmlInclude, jsCompile, mainJsCompile, scssCompile, libaryImages, libaryMovs)); // 세미 빌드 task
