...
jQuery(document).ready(function() {
	...
	function progressList() {
		...
		var stateFieldsST = $table.find("td.state.strategy");
		var stateFieldsDS = $table.find("td.state.design");
		var stateFieldsPB = $table.find("td.state.publishing");

		stateFieldsST.each(function(i, v) {
			var current = $(v);
			var prop = {
		...
		stateFieldsDS.each(function(i, v) {
			var current = $(v);
			var prop = {
				type: "../" + current.parents('.progress-list').attr('data-type'),
				url: current.siblings('.url').text()
		...
		stateFieldsPB.each(function(i, v) {
			var current = $(v);
			var prop = {
				type: "../" + current.parents('.progress-list').attr('data-type'),
				url: current.siblings('.url').text()
	...
	function dash_strategy() {
		...
		var $board = $('.progress-board.strategy');
		...
		$.each($lists, function(i) {
			...
			$item.find('.current').text( $list.find('.strategy.complete').length );
			$item.find('.total').text( $list.find('.strategy.state').length );
			$item.find('.percent').text( Math.floor($list.find('.strategy.complete').length / $list.find('.strategy.state').length * 100) + '%' );
	...
	function dash_design() {
		var $temp = $('<div class="progress"> <a href="javascript:void(0);"> <div class="title"> </div> <div class="stat"> <strong class="percent"></strong> <span class="count"> <span class="current"></span> <span class="slash">/</span> <span class="total"></span> </span> </div> </a> </div>');
		var $board = $('.progress-board.design');
		var $lists = $('.progress-list');
		...
		$.each($lists, function(i) {
			...
			$item.find('.title').text( $list.find('h2').text() );
			$item.find('.current').text( $list.find('.design.complete').length );
			$item.find('.total').text( $list.find('.design.state').length );
			$item.find('.percent').text( Math.floor($list.find('.design.complete').length / $list.find('.design.state').length * 100) + '%' );
			$item.find('a').attr( 'href', '#list' + _id );
			$list.attr( 'id', 'list' + _id );
	...
	function dash_publishing() {
		var $temp = $('<div class="progress"> <a href="javascript:void(0);"> <div class="title"> </div> <div class="stat"> <strong class="percent"></strong> <span class="count"> <span class="current"></span> <span class="slash">/</span> <span class="total"></span> </span> </div> </a> </div>');
		var $board = $('.progress-board.publishing');
		var $lists = $('.progress-list');
		...
		$.each($lists, function(i) {
			...
			$item.find('.title').text( $list.find('h2').text() );
			$item.find('.current').text( $list.find('.publishing.complete').length );
			$item.find('.total').text( $list.find('.publishing.state').length );
			$item.find('.percent').text( Math.floor($list.find('.publishing.complete').length / $list.find('.publishing.state').length * 100) + '%' );
			$item.find('a').attr( 'href', '#list' + _id );
			$list.attr( 'id', 'list' + _id );
